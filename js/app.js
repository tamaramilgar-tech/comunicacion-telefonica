document.addEventListener("DOMContentLoaded", () => {
  ...
  function refreshBadges() {
    // F1 -> habilita F2
    if (state.p1Passed) {
      if (phase1Badge) phase1Badge.textContent = `FASE 1: superada (${state.p1Score}%)`;
      if (phase2Tab) phase2Tab.disabled = false;
      if (phase2Badge) phase2Badge.textContent = state.p2Passed ? `FASE 2: superada (${state.p2Score}%)` : `FASE 2: pendiente`;
    } else {
      if (phase1Badge) phase1Badge.textContent = "FASE 1: pendiente";
      if (phase2Badge) phase2Badge.textContent = "FASE 2: bloqueada";
      if (phase2Tab) phase2Tab.disabled = true;
    }

    // F2 -> habilita F3
    if (state.p2Passed) {
      if (phase3Tab) phase3Tab.disabled = false;
      if (phase3Badge) phase3Badge.textContent = state.p3Passed ? `FASE 3: superada (${state.p3Score}%)` : `FASE 3: pendiente`;
    } else {
      if (phase3Badge) phase3Badge.textContent = "FASE 3: bloqueada";
      if (phase3Tab) phase3Tab.disabled = true;
    }

    // F3 -> habilita F4
    if (state.p3Passed) {
      if (phase4Tab) phase4Tab.disabled = false;
      if (phase4Badge) phase4Badge.textContent = state.p4Passed ? `FASE 4: superada (${state.p4Score}%)` : `FASE 4: pendiente`;
    } else {
      if (phase4Badge) phase4Badge.textContent = "FASE 4: bloqueada";
      if (phase4Tab) phase4Tab.disabled = true;
    }

    // Certificado
    if (certTab) certTab.disabled = !(state.p1Passed && state.p2Passed && state.p3Passed && state.p4Passed);
  }
  ...
  function sampleWithoutReplacement(items, n) {
    if (!Array.isArray(items)) throw new Error("Banco de preguntas no disponible.");
  ...
  function wireTeacherGate({ phaseId, inputId, btnId, msgId, submitBtnId, lockMsgId }) {
    ...
    // Si falta TeacherGate, no reventamos; solo mostramos bloqueo.
    const TG = window.TeacherGate;
    if (!TG || typeof TG.isUnlocked !== "function") {
      if (submitBtn) submitBtn.disabled = true;
      if (lockMsg) lockMsg.textContent = "Test bloqueado: falta el módulo de verificación del docente.";
    ...
    function refresh() {
      const ok = TG.isUnlocked(phaseId);
      if (submitBtn) submitBtn.disabled = !ok;
      if (lockMsg) lockMsg.textContent = ok ? "" : "Test bloqueado: requiere verificación del docente (entrega en EVAGD).";
      if (msg) msg.textContent = ok ? "✔ Verificación registrada (hoy)." : "";
    }
    ...
    if (verifyBtn) {
      verifyBtn.addEventListener("click", async () => {
        ...
        const ok = await TG.verify(phaseId, codeInput?.value || "", pin);
        if (ok) {
          TG.setUnlocked(phaseId);
          if (msg) msg.textContent = "✔ Código correcto. Test desbloqueado.";
          if (codeInput) codeInput.value = "";
          refresh();
        } else {
          if (msg) msg.textContent = "✖ Código incorrecto. Revisa el código o la fecha.";
        }
      });
    }
  ...
  // =====================
  // Estos bancos vienen de data.js (variables globales).
  const p1Attempt = makeAttempt(window.phase1Bank || phase1Bank, 10);
  const p2Attempt = makeAttempt(window.phase2Bank || phase2Bank, 10);
  const p3Attempt = makeAttempt(window.phase3Bank || phase3Bank, 10);

  // Fase 4 solo si existe el banco
  const hasPhase4 = (typeof window.phase4Bank !== "undefined") || (typeof phase4Bank !== "undefined");
  const p4Attempt = hasPhase4 ? makeAttempt(window.phase4Bank || phase4Bank, 10) : [];

  renderQuiz("p1Quiz", p1Attempt, "p1");
  renderQuiz("p2Quiz", p2Attempt, "p2");
  renderQuiz("p3Quiz", p3Attempt, "p3");
  if (hasPhase4) renderQuiz("p4Quiz", p4Attempt, "p4");
  ...
  if (p4Submit) {
    p4Submit.addEventListener("click", () => {
      if (!hasPhase4) {
        alert("La Fase 4 no está configurada (falta el banco phase4Bank en data.js).");
        return;
      }
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener("click", () => {
      ...
      if (typeof generateCertificatePDF !== "function") {
        alert("No se pudo generar el certificado: falta certificate.js.");
        return;
      }

      generateCertificatePDF({
        studentName,
        unitName: typeof UNIT_NAME !== "undefined" ? UNIT_NAME : "Unidad",
